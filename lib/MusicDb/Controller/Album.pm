package MusicDb::Controller::Album;
...
sub view_details {
	...
	my $instance = MusicDb::Model::Album->_get($id);
	...
	my $artist = MusicDb::Model::Artist->_get_all({id => $artistid});
	my $artist_name = $artist->[0]->{name};
	
	my $genre = MusicDb::Model::Genre->_get_all({id => $genreid});
