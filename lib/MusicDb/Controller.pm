...
use MusicDb::Model;
use MusicDb::UI::Tabs;
use MusicDb::Controller::Artist;
use MusicDb::Controller::Album;
use MusicDb::Controller::Track;
use MusicDb::Controller::Genre;
...
our $handlers = {
	'artist' => 'MusicDb::Controller::Artist',
	'album' => 'MusicDb::Controller::Album',
	'track' => 'MusicDb::Controller::Track',
	'genre' => 'MusicDb::Controller::Genre',
};
...
sub submit {
	...
	my $object = {};
	my $lib = 'MusicDb::Model::' . ucfirst($model);
...
sub add {
	...
	my $action = '/' . $model . '/add';
	my $metadata = $lib->_metadata();	
	my $form = new MusicDb::UI::Form($model, $metadata, $action);
...
sub edit {
	...
	if ($model eq 'biography') {
		...
		$bio = MusicDb::Helper->parse_bio($bio);
...
sub list_model {
	...
	my $columns = MusicDb::Helper->get_columns($model);
	
	if (session('user') eq 'cristi') {
		my $commands = $columns->[1]->{command};
		push @$commands, 'edit';
	}
	my $grid = new MusicDb::UI::Grid({
			datasource => {
					data => $ds,
					pageSize => 10,
			},
